...
# from SAT import sat_main
from output_dep_tree import change_dependency_structure_json
import os
from ast import literal_eval
import test
import solution_similarity
from SAT_risky import DepGraph_sat_risky,solver_sat_risky,output_depinfo_in_json_risky,sat_main_risky
...
def main(obj_parameter_list,obj_parameter_str):
    ...
    file_url = "C:/Users/15040/Desktop/Ground_Truth_Dataset/Ground_Truth_Dataset_ALL_RISKY.txt"
        ...
        for line in lines:
            ...
            try:
                if line == "#################################################################################################################":
                    ...
                    count_ststus = count_result(repository_full_name, html_url, title_info)

                    if repository_full_name != "" and must_list!="[]" and count_ststus==0:
                        print("#################################################################################################################")
                        ...
                        for developer_dep_list_only_with_must_info in SAT_MUST_solution_dep_list:
                            ...
                            if select_pacakgeType(developer_dep_list_only_with_must_name,developer_dep_list_only_with_must_version) == "True" \
                                    ...
                                    or "pre" in developer_dep_list_only_with_must_version :
                        ...
                        if os.path.exists(file_url) == False:
                            # 将依赖文件写入json格式
                            output_depinfo_in_json_risky.def_info_in_json(after_commits_targetFramework, json_trigger_dep_list, json_file_name)
                        # 执行SAT
                        best_solution_dep_list = sat_main_risky.start_SAT(json_file_name,obj_parameter_list)
                        solution_num=1
                        ...
                        SAT_count_NU1103 = 0
                        for SAT_dep_list_only_with_must_info in best_solution_dep_list:
                            SAT_dep_list_only_with_must_name = \
                            SAT_dep_list_only_with_must_info.split("@")[0]
                            SAT_dep_list_only_with_must_version = \
                            SAT_dep_list_only_with_must_info.split("@")[1]

                            if select_pacakgeType(SAT_dep_list_only_with_must_name,
                                                  SAT_dep_list_only_with_must_version) == "True" \
                                    or select_pacakgeType(SAT_dep_list_only_with_must_name,
                                                          SAT_dep_list_only_with_must_version) == "" \
                                    or "pre" in SAT_dep_list_only_with_must_version:
                                SAT_count_NU1103 = SAT_count_NU1103 + 1
                        ...
                        write_file("SAT-NU1103: " + str(SAT_count_NU1103))
                        ...
                        install_sat_result(repository_full_name, html_url, title_info, "",
                                           after_commits_targetFramework, len(first_dep_list), total_dep_count,
                                           NU1605_count, NU1107_count, NU1202_count,warning_count, count_NU1103,
                                           0,
                                           developer_directly_dependencies_count, developer_total_dep_count,
                                           developer_building_dependency_tree_return_list[8], count_NU1103, 0,
                                           developer_major_count, developer_change_directly_count,
                                           len(best_solution_dep_list), SAT_total_dep_count,
                                           SAT_building_dependency_tree_return_list[8], SAT_count_NU1103, 0,
                                           SAT_major_count, SAT_change_directly_count,
                                           developer_directly_dependencies_count-len(best_solution_dep_list),
                                           difference_value_total_dep_count,
                                           difference_value_warning_count,
                                           difference_value_major_count,
                                           difference_value_change_directly_count,
                                           obj_parameter, same_packageName_proportion,
                                           same_packageMajorVersion_proportion, same_dependencies_proportion)
...
def install_sat_result(repository_full_name,html_url,title_info,commit_date,
                       targetFramework,directly_dependencies_count,total_dep_count,
                       NU1605_count,NU1107_count,NU1202_count,NU1608_count,NU1103_count,NU1701_count,
                       developer_directly_dependencies_count,developer_total_dep_count,
                       developer_NU1608_count,developer_NU1103_count,developer_NU1701_count,
                       developer_major_count,developer_change_directly_count,
                       SAT_directly_dependencies_count,SAT_total_dep_count,
                       SAT_NU1608_count,SAT_NU1103_count,SAT_NU1701_count,
                       SAT_major_count,SAT_change_directly_count,
                       difference_value_directly_dependencies_count,
                       difference_value_total_dep_count,
                       difference_value_warning_count,
    ...
    sql_insert = "INSERT INTO ground_truth_dataset_risky_0423 (repository_full_name,html_url,title_info,commit_date," \
                       "targetFramework,directly_dependencies_count,total_dep_count," \
                       "NU1605_count,NU1107_count,NU1202_count,NU1608_count,NU1103_count,NU1701_count," \
                       "developer_directly_dependencies_count,developer_total_dep_count," \
                       "developer_NU1608_count,developer_NU1103_count,developer_NU1701_count," \
                 ...
                 % (repository_full_name,html_url,title_info,commit_date,
                       targetFramework,directly_dependencies_count,total_dep_count,
                       NU1605_count,NU1107_count,NU1202_count,NU1608_count,NU1103_count,NU1701_count,
                       developer_directly_dependencies_count,developer_total_dep_count,
                       developer_NU1608_count,developer_NU1103_count,developer_NU1701_count,
...
def count_result(repository_full_name, html_url,title_info):
    sql = "SELECT COUNT(*) FROM ground_truth_dataset_risky_0423 WHERE repository_full_name = '%s' and html_url = '%s' and title_info = '%s'" % (
    repository_full_name, html_url,title_info)
    try:
        # 执行sql语句
        ...
        print("find_nuget_all_releases_count() dberror", e)
...
if __name__ == '__main__':
    ...
    obj_parameter_05 = [0.06064, 0.01047931,0.0092341,0.000411719 , 0.007152835 , 0.077190642 ,0.834891371]
    obj_parameter_list=[]
    ...
    for obj_parameter_info in obj_parameter_list:
        ...
        file_name = "Ground_Truth_Dataset_Details"+obj_parameter_str+"-test0423-新系数_risky"
