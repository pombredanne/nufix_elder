...
def search_dep(dependency_name,dependency_version_range,direct_dep,level,times):
    ...
    if temp_search_info!=[]:
        ...
        installed_dep_version_range_list = temp_search_info[2]
        ...
        if check_installed_version=="":
            version_ststus = 0
            conflict_installed_version_range=""
            for installed_dep_version_range in installed_dep_version_range_list:
                # 检查新的版本是否在已安装依赖范围内
                ...
                if check_dependency_version == "":
                    version_ststus=1
                    conflict_installed_version_range = installed_dep_version_range
            ...
            if version_ststus==0:
                ...
                new_dependency_version_range_list = installed_dep_version_range_list
                new_dependency_version_range_list.append(dependency_version_range)

                new_install_dep_list.append(new_dependency_version_range_list)
                ...
                if semver.compare(dependency_version, installed_dep_version) == -1:
                    ...
                    temp_installed_direct_dep_name = installed_dep_direct_dep.split("@")[0]
                    temp_installed_direct_dep_version = matching_version.intercept_version(installed_dep_direct_dep.split("@")[1])
                    temp_installed_direct_dep_info = temp_installed_direct_dep_name + "@" + temp_installed_direct_dep_version

                    # if "Microsoft.AspNetCore.JsonPatch"==dependency_name:
                    #     print("1")
                    #     print("dependency_name+@+installed_dep_version： ",dependency_name+"@"+installed_dep_version)
                    ...
                    if dependency_name+"@"+dependency_version == direct_dep:
                        ...
                        str_print("间接依赖范围："+ conflict_installed_version_range)
                    ...
                    # 低版本为间接依赖
                    elif dependency_name+"@"+installed_dep_version == temp_installed_direct_dep_info:
                        # print("*****************************************************************")
                    ...
                    # 高低版本呢均为间接依赖
                        ...
                        str_print("冲突间接依赖范围："+conflict_installed_version_range)
                        ...
                        for global_search_info in global_search_list:
                            if global_search_info[0]==dependency_name:
                                temp_range_list = global_search_info[2]
                                temp_range_list.append(dependency_version_range)
                                global_search_info[2] = temp_range_list
                    ...
                    # 低版本为间接依赖
                    # 高版本为直接依赖 intercept_version
                    temp_direct_dep_name = direct_dep.split("@")[0]
                    temp_direct_dep_version = matching_version.intercept_version(direct_dep.split("@")[1])
                    temp_direct_dep_info = temp_direct_dep_name + "@" + temp_direct_dep_version
                    ...
                    elif dependency_name + "@" + dependency_version == temp_direct_dep_info:
                        print("*****************************************************************")
                        print("error! NU1608 直接依赖版本高于间接依赖，但无交集",dependency_name)
                        print("直接版本：",dependency_version)
                        print("间接依赖范围：", conflict_installed_version_range)
    ...
    # 无同名依赖包，正常执行安装
        ...
        dependency_version_range_list = []
        dependency_version_range_list.append(dependency_version_range)

        new_install_dep_list.append(dependency_version_range_list)
        new_install_dep_list.append(children_name_list)
        new_install_dep_list.append(direct_dep)
        new_install_dep_list.append(matched_framework)
...
def building_dependency_tree(dep_targetFramework,direct_dep_list,global_list):
    ...
    # print("NU1605", error_type_count_dict['NU1605'])
    # print("NU1107", error_type_count_dict['NU1107'])
    # print("NU1202", error_type_count_dict['NU1202'])
    # print("warning_count", warning_count)


    for global_search in global_search_list:
        if global_search[0]=="Microsoft.AspNetCore.JsonPatch":
            print(global_search[2])
...
if __name__ == '__main__':
    ...
    # direct_dep_list=['Consul@1.6.1', 'FluentValidation@8.5.1', 'FluentValidation.AspNetCore@8.5.1', 'MediatR@8.0.0', 'MediatR.Extensions.Microsoft.DependencyInjection@7.0.0', 'Microsoft.AspNetCore.Mvc.NewtonsoftJson@3.0.0', 'Microsoft.EntityFrameworkCore.SqlServer@3.0.0', 'Microsoft.EntityFrameworkCore.Tools@3.0.0', 'Microsoft.Extensions.Options.ConfigurationExtensions@3.0.1', 'RawRabbit@1.10.1', 'Serilog.AspNetCore@3.2.0', 'Serilog.Exceptions@5.4.0', 'Consul@1.6.1']

    direct_dep_list=['OpenMod.API@1.1.6', 'OpenMod.Core@1.1.6', 'OpenMod.UniTask@2.0.19', 'OpenMod.UnityEngine@1.1.6', 'OpenMod.Unturned@1.1.9', 'OpenMod.Unturned.Redist@3.20.7', 'NSec.Cryptography@18.2.0-preview1','System.Runtime.CompilerServices.Unsafe@5.0.0']
    ...
    print("引入兼容框架个数",return_list[1])
    print("引入发布不规范的risky package的个数",return_list[2])
    print("引入良性依赖冲突的个数(不报error)",return_list[3])
    print("冲突直接依赖列表", return_list[4])
    print("NU1605", return_list[5])
    print("NU1107", return_list[6])
    print("NU1202", return_list[7])
    print("warning_count", return_list[8])
