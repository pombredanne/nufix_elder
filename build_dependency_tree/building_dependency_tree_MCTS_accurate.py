    ...
    if key in global_dep_info_dict:
        ...
        if commitTimeStamp_pacakgeType_list!=[]:
            ...
            if matched_framework is None or matched_framework == "":
                # print("*****************************************************************")
                # print("error! NU1202 框架匹配冲突")
                # print("直接依赖及其版本：", direct_dep)
                # print("已安装版本：", dep_version)
...
def search_dep(dependency_name,dependency_version_range,direct_dep,level,times):
    ...
    if level>6:
        return
    level=+1
    # print(dependency_name, dependency_version_range)
    ...
    if temp_search_info!=[]:
        ...
        if check_installed_version=="":
            ...
            if check_dependency_version!="":
                # print("有交集，需要回溯")
                # print(installed_dep_name, installed_dep_version, installed_dep_version_range,
                #       installed_dep_children_name_list, installed_dep_direct_dep)
                # print(dependency_name, dependency_version_range, direct_dep)
                ...
                # if dependency_name == "Microsoft.Extensions.DependencyInjection":
                #     print("Microsoft.Extensions.DependencyInjection")
                #     # installed_dep_version = temp_search_info[1]
                #     # installed_dep_version_range = temp_search_info[2]
                #     # installed_dep_children_name_list = temp_search_info[3]
                #     # installed_dep_direct_dep = temp_search_info[4]
                #     print(dependency_name, dependency_version,dependency_version_range, direct_dep)
                #     print(installed_dep_version, installed_dep_version_range, installed_dep_children_name_list,installed_dep_direct_dep)
                ...
                if semver.compare(dependency_version, installed_dep_version) == -1:
                    ...
                    if dependency_name+"@"+dependency_version == direct_dep:
                        # print("*****************************************************************")
                        # print("error! NU1605 直接依赖版本版本低于间接依赖", dependency_name)
...
def building_dependency_tree(dep_targetFramework,direct_dep_list,global_list):
    ...
    for direct_dep in direct_dep_list:
        ...
        direct_dependency_name = direct_dependencies_info_list[0]
        direct_dependency_version_range = direct_dependencies_info_list[1]
