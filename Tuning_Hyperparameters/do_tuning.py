...
def get_training_dataset_no_risky():
    ...
    sql = "select id,repository_full_name,title_info,commit_date," \
          "developer_total_dep_count,developer_major_count,developer_change_directly_count,directly_dependencies_count from ground_truth_dataset " \
          "where SAT_NU1608_count=0 and SAT_NU1103_count=0 and SAT_NU1701_count=0 and id <=9231 "
    try:
...
def black_box_function(a1,a2,a3,a4):
    ...
    for return_info in return_list:
        try:
            ...
            print(repository_full_name, title_info, commit_date)

            json_file_name = repository_full_name.replace(r"/", "-") + "_" \
                             + title_info.split(r"/")[-1].replace(r"/", "-").replace(r".", "_") + "_" \
                             + str(commit_date).replace(r"T", "-").replace(r":", "-")

            sat_values = start_SAT(json_file_name, obj_parameter_list)
            ...
            developer_values = a1*(directly_dependencies_count - developer_major_count)-a2*(developer_change_directly_count) - a4*(developer_total_dep_count)
...
def main():
    ...
    pbounds = {'a1': (0.6, 1), 'a2': (0.1, 0.3),'a3': (0.1, 0.3),'a4': (0, 0.02)}
